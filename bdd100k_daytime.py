...
class bdd100k_gan(imdb):
    def __init__(self,image_set,  use_diff=False):
        imdb.__init__(self, "bdd100k" + "_gan_" + image_set)
        ...
        self._data_path = os.path.join(self._devkit_path, 'images', '100k',self.mode)
    ...
    def _get_default_path(self):
        ...
        return os.path.join(cfg.bdd_gan)
    ...
    def gt_roidb(self):
        ...
        gt_ann = {}
        with open(os.path.join(self._devkit_path, 'labels', 'bdd100k_labels_images_%s.json' % self.mode), 'rb') as f:
            annots = json.load(f)
            for ann in annots:
                if ann['attributes']["timeofday"] == "daytime":
                    gt_ann[ann['name']] = ann['labels']
        ...
        for index in self.image_index:
            try:
                gt_roidb.append(self._load_bdd100k_annotation(gt_ann[index]))
            except KeyError:
                continue
